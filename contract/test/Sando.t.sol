...
import "./helpers/SandoHelper.sol";
...
contract SandoTest is Test {
    ...
    address constant searcher = 0x6E5F4552091a69125D5DfCB7B8c2659029395Bdf;
    ...
    function setUp() public {
        // deploy and fund sandwich
        sandwich = HuffDeployer.deploy("sando");
        weth.deposit{value: wethFundAmount}();
        weth.transfer(sandwich, wethFundAmount);

        // pranked addresses need to be funded to cover their gas fees
    ...
    //        sandwichHelper.v3CreateSandwichPayloadWethIsInput(pool, inputToken, outputToken, fee, amountIn);
    ...
    //        sandwichHelper.v3CreateSandwichPayloadWethIsOutput(pool, inputToken, outputToken, fee, amountIn);
    ...
    //    // fund sandwich contract
    //    vm.prank(binance8);
    //    ERC20(inputToken).transfer(sandwich, uint256(amountIn));
    ...
    //    vm.deal(address(0xf337babe), 100 ether);
    //    vm.startPrank(address(0xf337babe));

    //    string memory functionName = "recoverEth";
    //    bytes memory payload = abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName));
    //    (bool s,) = sandwich.call(payload);

    //    assertFalse(s, "unauthorized addresses should not call recover eth");

    //    functionName = "recoverWeth";
    //    payload = abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName));
    //    (s,) = sandwich.call(payload);

    //    assertFalse(s, "unauthorized addresses should not be able to call recover weth");

    //    functionName = "seppuku";
    //    payload = abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName));
    //    (s,) = sandwich.call(payload);

    //    assertFalse(s, "unauthorized addresses should not be able to seppuku contract");
    //    changePrank(searcher);
    //    (s,) = sandwich.call(payload);
    ...
    //    string memory functionName = "recoverEth";
    //    emit log_bytes(abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName)));
    //    bytes memory payload = abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName));
    //    (bool s,) = sandwich.call(payload);
    ...
    //    assertTrue(sandwichBalanceAfter == 0, "sandwich eth balance should be zero");
    //    assertTrue(
    //        searcherBalanceAfter == searcherBalanceBefore + sandwichBalanceBefore,
    ...
    //    bytes memory payload = abi.encodePacked(sandwichHelper.getJumpLabelFromSig(functionName), sandwichBalanceBefore);
    //    (bool s,) = sandwich.call(payload);
    //    assertTrue(s, "calling recoverWeth failed");

    //    uint256 sandwichBalanceAfter = weth.balanceOf(sandwich);
    //    uint256 searcherBalanceAfter = weth.balanceOf(searcher);
    ...
    //    uint256 usdtBalanceBefore = ERC20(outputToken).balanceOf(sandwich);
    ...
    //    (bytes memory payloadV4, uint256 encodedValue) =
    //        sandwichHelper.v2CreateSandwichPayloadWethIsInput(outputToken, amountIn);
    //    vm.prank(searcher);
    ...
    //    uint256 usdtBalanceChange = ERC20(outputToken).balanceOf(sandwich) - usdtBalanceBefore;
    ...
    //    // Pre swap checks
    //    uint256 wethBalanceBefore = weth.balanceOf(sandwich);
    //    uint256 usdcBalanceBefore = ERC20(outputToken).balanceOf(sandwich);
    ...
    //    // Check values after swap
    //    uint256 wethBalanceChange = wethBalanceBefore - weth.balanceOf(sandwich);
    //    uint256 usdcBalanceChange = ERC20(outputToken).balanceOf(sandwich) - usdcBalanceBefore;
    ...
    //    // Pre swap checks
    //    uint256 wethBalanceBefore = weth.balanceOf(sandwich);
    //    uint256 superFarmBalanceBefore = ERC20(inputToken).balanceOf(sandwich);
    ...
    //        (amountOutFromEncoded / sandwichHelper.wethEncodeMultiple()) * sandwichHelper.wethEncodeMultiple();

    //    // Perform swap
    //    (bytes memory payloadV4, uint256 encodedValue) =
    //        sandwichHelper.v2CreateSandwichPayloadWethIsOutput(inputToken, amountIn);
    ...
    //    uint256 superFarmBalanceChange = superFarmBalanceBefore - ERC20(inputToken).balanceOf(sandwich);
    ...
    //    // Fund sandwich
    //    vm.prank(binance8);
    //    ERC20(inputToken).transfer(sandwich, amountIn);

    //    // Pre swap checks
    //    uint256 wethBalanceBefore = weth.balanceOf(sandwich);
    //    uint256 daiBalanceBefore = ERC20(inputToken).balanceOf(sandwich);
    ...
    //    // Check values after swap
    //    uint256 wethBalanceChange = weth.balanceOf(sandwich) - wethBalanceBefore;
    //    uint256 daiBalanceChange = daiBalanceBefore - ERC20(inputToken).balanceOf(sandwich);
